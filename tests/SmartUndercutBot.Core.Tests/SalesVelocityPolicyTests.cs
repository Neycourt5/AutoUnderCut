...
public sealed class SalesVelocityPolicyTests
{
    ...
    [Fact]
    public void DailyUnitsOutrankCategoryAndLargerProfitWhenOneSlotIsFree()
    {
        var now = DateTimeOffset.UtcNow;
        var markets = new[] {
            new ProcurementMarketItem(1, "Slow food", [new(1, 11, 21, "Cactuar", 1, 100, 10, false)],
                [new(2000, 700, false, now.AddDays(-1))], NqSalesPerDay: 2),
            new ProcurementMarketItem(2, "Fast dye", [new(2, 12, 22, "Cactuar", 1, 100, 10, false)],
                [new(1000, 70, false, now.AddDays(-1))], NqSalesPerDay: 500),
        };
        var rules = new[] {
            new ProcurementRule { ItemId = 1, ItemName = "Slow food", TourPriority = 0, TargetStackSize = 10 },
            new ProcurementRule { ItemId = 2, ItemName = "Fast dye", TourPriority = 5, TargetStackSize = 10 },
        };
